...
export default function Studio({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="app-shell studio-app" lang="vi"><nav className="topbar"><Brand suffix="STUDIO" /><a className="back-link" href="/"><Icon name="back" />Bảng điều khiển</a></nav>
    ...
    {!data ? <section className="panel"><Empty icon={loading ? "refresh" : "alert"}>{loading ? "Đang tải studio…" : "Không mở được dự án. Dự án có thể đã bị xóa hoặc API chưa sẵn sàng."}</Empty><button disabled={busy || loading} onClick={() => void perform(load, "Đã tải dự án.", false)}><Icon name="refresh" />Thử lại</button></section> : <>
      ...
      {tab === "deck" && <><header className="section-head"><div><h2>Bàn điều khiển</h2><p className="muted">12 vị trí mỗi trang. Bấm nút để phát trên overlay, không phát trong studio.</p></div><div className="actions"><button className="ghost compact" onClick={() => void stopCurrent()}><Icon name="stop" />Dừng hiện tại</button><button className="danger compact" onClick={() => void clearQueue()}><Icon name="clear" />Xóa hàng đợi</button><button disabled={!data.effects.length || buttonEditor !== null} onClick={() => setButtonEditor("new")}><Icon name="plus" />Nút</button></div></header>
        ...
        <div className="pagination"><button className="ghost compact" disabled={currentPage === 0} onClick={() => setPage(currentPage - 1)}><Icon name="back" />Trước</button><Field label={`Trang / ${pageCount}`}><input aria-label="Trang bàn điều khiển" type="number" min={1} max={pageCount} value={currentPage + 1} onChange={event => setPage(Math.max(0, Math.min(pageCount - 1, Number(event.target.value) - 1)))} /></Field><button className="ghost compact" disabled={currentPage >= pageCount - 1} onClick={() => setPage(currentPage + 1)}>Sau<Icon name="back" style={{ transform: "rotate(180deg)" }} /></button></div>
